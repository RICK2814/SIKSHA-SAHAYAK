...
def practice_questions(request, chapter_id=None):
    ...
    attempted_ids = PracticeAttempt.objects.filter(student=request.user, question__in=questions).values_list('question_id', flat=True)
    ...
    if request.method == 'POST':
        ...
        PracticeAttempt.objects.create(student=request.user, question=question, selected_answer=selected_answer, is_correct=is_correct)
        return render(request, 'assessments/practice_result.html', {'question': question, 'selected_answer': selected_answer, 'is_correct': is_correct, 'chapter': chapter})

    question = questions.first() if questions.exists() else None
    return render(request, 'assessments/practice_questions.html', {'question': question, 'chapter': chapter, 'total_available': questions.count()})
...
def quiz_detail(request, quiz_id):
    ...
    return render(request, 'assessments/quiz_detail.html', {'quiz': quiz, 'questions': questions, 'attempts': attempts})
...
def take_quiz(request, quiz_id):
    ...
    if request.method == 'POST':
        attempt = QuizAttempt.objects.create(student=request.user, quiz=quiz, total_questions=quiz_questions.count())
        score = 0
        for qq in quiz_questions:
            ...
            if is_correct:
                score += qq.marks
            StudentAnswer.objects.create(attempt=attempt, question=qq.question, selected_answer=selected or '', is_correct=is_correct)
        attempt.score = score
    ...
    return render(request, 'assessments/take_quiz.html', {'quiz': quiz, 'quiz_questions': quiz_questions})

@login_required
def quiz_result(request, attempt_id):
    attempt = get_object_or_404(QuizAttempt, id=attempt_id, student=request.user)
    answers = attempt.answers.select_related('question').all()
    return render(request, 'assessments/quiz_result.html', {'attempt': attempt, 'answers': answers})
...
def my_progress(request):
    ...
    profile = StudentProfile.objects.filter(user=request.user).first()
    subjects = Subject.objects.filter(class_level=profile.class_level) if profile and profile.class_level else []
    recent_attempts = quiz_attempts.filter(completed_at__isnull=False).order_by('-completed_at')[:10]
    return render(request, 'assessments/my_progress.html', {'total_quizzes': total_quizzes, 'completed_quizzes': completed_quizzes, 'avg_score': round(avg_score, 2), 'highest_score': highest_score, 'total_questions': total_questions, 'correct_answers': correct_answers, 'accuracy': accuracy, 'subjects': subjects, 'recent_attempts': recent_attempts})
...
def dashboard(request):
    ...
    correct_answers = practice_attempts.filter(is_correct=True).count()
    topics_completed = Chapter.objects.filter(subject__class_level=profile.class_level).count() if profile and profile.class_level else 0
    context = {'profile': profile, 'subjects': subjects, 'quizzes_attempted': quizzes_attempted, 'avg_score': round(avg_score, 2), 'topics_completed': topics_completed, 'questions_attempted': questions_attempted, 'correct_answers': correct_answers}
